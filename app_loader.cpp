...
{
  int rc = -1;
  auto startAddr = user_space_->getStartAddr();
  // Load RTLD (ld.so)
  ...
  void* newStack = user_space_->createNewStack(ldso, stackStartAddr, param_index, param_count, socket_id);
  ...
  void* newHeap = user_space_->createNewHeap(heapStartAddr);
  if (!newHeap) {
